...
export const GlareCard = ({
  ...
  const updateStyles = () => {
    if (refElement.current) {
      ...
      refElement.current?.style.setProperty("--opacity", "0.3"); // Subtler visibility
    }
  };
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const angle = time * 0.7; // Slower, more subtle rotation

      // Simulate movement - reduced range
      const offsetX = Math.sin(angle) * 20; // -20 to 20
      const offsetY = Math.cos(angle * 0.8) * 20;

      // Update state for auto animation
      state.current.rotate.x = offsetY * 0.3;
      state.current.rotate.y = offsetX * 0.3;

      state.current.glare.x = 50 + offsetX;
      state.current.glare.y = 50 + offsetY;

      state.current.background.x = 50 + offsetX;
      state.current.background.y = 50 + offsetY;

      updateStyles();
      animationFrameId = requestAnimationFrame(animate);
    };

    // Start animation immediately (can be gated by a prop if needed, but user wants "always on" for mobile feel)
    const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
    if (isMobile || true) { // Force enable for this demo
      animate();
    }
  ...
  return (
    <div
      ...
      className="relative isolate w-full h-full transition-transform delay-[var(--delay)] duration-[var(--duration)] ease-[var(--easing)] will-change-transform [contain:layout_style] [perspective:600px]"
      ref={refElement}
      onPointerMove={(event) => {
